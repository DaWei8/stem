...
export default function ProjectEditorPage() {
  ...
  const {
    tables,
    columns,
    fetchProjectData: fetchDatabaseData,
  } = useDatabase();
  ...
  const exportOptions = [
    {
      ...
      description:
        "A beautifully formatted MS Word document of your entire project.",
      action: () => {
        const data = {
        ...
        downloadDocFile(
          docText,
          `${currentProject?.name?.toLowerCase().replace(/\\s+/g, "_") || "project"}_docs.doc`,
        );
    ...
    },
    {
      ...
      description:
        "The entire deterministic project state including all pillars.",
      action: () => {
        const data = {
          project: currentProject,
          architecture: { pages, transitions, inputs, actions, outputs },
        ...
        downloadFile(
          data,
          `${currentProject?.name?.toLowerCase().replace(/\\s+/g, "_") || "project"}.stem`,
        );
        ...
        return (
          <div className="flex items-center justify-center h-full text-zinc-600 bg-zinc-50 dark:bg-black transition-colors duration-300">
            <div className="text-center space-y-4">
              ...
              <div>
                <h3 className="text-sm font-black text-zinc-600 dark:text-zinc-400">
                  Pillar: {activeView}
                </h3>
                <p className="text-[10px] font-medium text-zinc-400 dark:text-zinc-600 mt-1 tracking-tighter">
                  This deterministic module is under construction.
                </p>
              </div>
            </div>
          </div>
    ...
    <div className="flex-1 h-screen w-full relative bg-zinc-50 dark:bg-black overflow-hidden flex flex-col selection:bg-black/10 dark:selection:bg-white/20 transition-colors duration-300">
      ...
      >
        <div className="flex items-center gap-3">
          <Button
            href="../projects/"
            className="bg-zinc-100 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-800 text-black dark:text-white size-7 rounded-md p-0 group hover:bg-black hover:text-white dark:hover:bg-white dark:hover:text-black transition-all"
          >
        ...
        <div className="flex items-center gap-3">
          ...
          {isViewer ? (
            ...
            <Button
              ...
              onClick={() =>
                toast.info("Auto-save enabled. Registry state is persistent.")
              }
            >
      ...
      >
        <div className="flex flex-col gap-3 py-4">
          {exportOptions.map((opt, i) => (
            ...
            >
              <span className="text-sm font-bold text-black dark:text-white group-hover:text-black dark:group-hover:text-white transition-colors">
                {opt.label}
              </span>
              <span className="text-xs text-zinc-500 dark:text-zinc-400 mt-1 transition-colors">
                {opt.description}
              </span>
            </button>
          ))}
