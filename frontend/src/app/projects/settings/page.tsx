...
export default function SettingsPage() {
  ...
  const systemTokens = useMemo(
    () => totals.byProvider["system-fallback"]?.tokens || 0,
    [totals],
  );
  const systemTokensLimit = 50000;
  const systemTokensPercent = useMemo(
    () => Math.min((systemTokens / systemTokensLimit) * 100, 100),
    [systemTokens],
  );
  ...
  const handleTestKey = async (
    ...
    if (!key.trim()) {
      toast.error(
        `Please enter an API Key for ${provider === "google" ? "Google Gemini" : provider} first.`,
      );
      return;
    }
    ...
    try {
      ...
      if (data.ok) {
        ...
        toast.error(
          `${provider.toUpperCase()} Key verification failed: ${data.error || "Invalid credentials"}`,
        );
      }
    } catch (err: any) {
      setStatus("failed");
  ...
  const selectModel = (modelId: string, isUnlocked: boolean) => {
    ...
    toast.success(
      `Active Architect set to: ${modelId} (click Save to persist)`,
    );
  };
  ...
  const getProviderBadge = (provider: string) => {
    switch (provider) {
      case "openai":
        return (
          <span className="px-2 py-0.5 border border-sky-500/20 bg-sky-500/10 text-sky-400 font-mono text-[9px] font-bold uppercase">
            OpenAI
          </span>
        );
      case "anthropic":
        return (
          <span className="px-2 py-0.5 border border-amber-500/20 bg-amber-500/10 text-amber-400 font-mono text-[9px] font-bold uppercase">
            Anthropic
          </span>
        );
      case "google":
        return (
          <span className="px-2 py-0.5 border border-emerald-500/20 bg-emerald-500/10 text-emerald-400 font-mono text-[9px] font-bold uppercase">
            Gemini Client
          </span>
        );
      default:
        return (
          <span className="px-2 py-0.5 border border-zinc-700 bg-zinc-800/50 text-zinc-400 font-mono text-[9px] font-bold uppercase">
            STEM API
          </span>
        );
    }
    ...
    <div className="flex flex-col min-h-screen bg-black text-white font-heading">
      <header className="flex items-center justify-between px-8 py-6 border-b border-zinc-800 bg-black/50 backdrop-blur-md sticky top-0 z-10">
        <div className="flex items-center gap-4">
          <Link href="/projects">
            <Button
              size="icon"
              className="bg-white/10 border border-zinc-800 hover:bg-zinc-800 rounded-md size-8"
            >
              <ChevronLeft className="size-4 text-white" />
            </Button>
          </Link>
          <h1 className="text-sm font-bold text-zinc-500 ">
            Global / Settings
          </h1>
        </div>
      ...
      <main className="max-w-6xl mx-auto w-full p-8 space-y-12 pb-20">
        ...
        <section className="space-y-6">
          ...
          <div className="p-6 rounded-xl border border-zinc-900 bg-black/30 space-y-4">
            <div className="flex justify-between items-center">
              ...
              <Button
                variant="outline"
                className="rounded-md border-zinc-800 h-9 px-4 text-[10px] font-bold hover:bg-white hover:text-black transition-all"
              >
                Enable 2FA
              </Button>
            </div>
            ...
            <div className="flex justify-between items-center">
              <div>
                ...
                <p className="text-[10px] text-zinc-500 font-medium mt-1">
                  Manage keys for programmatic system access.
                </p>
              </div>
              <Button
        ...
        <section className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div className="p-6 border border-zinc-900 bg-black/30 space-y-4">
              ...
              <div className="space-y-1">
                <h3 className="text-xl font-black">
                  {projects.length} / {profile?.max_projects || 10}
                </h3>
                <div className="w-full h-1 bg-black rounded-full overflow-hidden">
                  <div
                    className="bg-white h-full"
                    style={{
                      width: `${Math.min((projects.length / (profile?.max_projects || 10)) * 100, 100)}%`,
                    }}
                  />
                </div>
              </div>
            ...
            <div className="p-6 border border-zinc-900 bg-black/30 space-y-4">
              ...
              <div className="space-y-1">
                <h3 className="text-xl font-black">
                  {uniqueCollaborators} / {profile?.max_collaborators || 3}
                </h3>
                <div className="w-full h-1 bg-black rounded-full overflow-hidden">
                  <div
                    className="bg-white h-full"
                    style={{
                      width: `${Math.min((uniqueCollaborators / (profile?.max_collaborators || 3)) * 100, 100)}%`,
                    }}
                  />
                </div>
              </div>
            ...
            <div className="p-6 border border-zinc-900 bg-black/30 space-y-4">
              <p className="text-[10px] font-black text-zinc-600 ">
                AI Tokens (STEM Default)
              </p>
              <div className="space-y-1">
                <h3 className="text-xl font-black">
                  {formatTokens(systemTokens)} /{" "}
                  {formatTokens(systemTokensLimit)}
                </h3>
                <div className="w-full h-1 bg-black rounded-full overflow-hidden">
                  <div
                    className="h-full bg-emerald-500 transition-all duration-300"
                    style={{ width: `${systemTokensPercent}%` }}
                  />
                </div>
              </div>
            </div>
        ...
        <section className="space-y-6">
          ...
          <p className="text-[11px] text-zinc-400 font-medium">
            Monitor prompt tokens, output completion metrics, and calculated
            session costs across your custom keys and standard blueprints.
          </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="p-6 border border-zinc-800 bg-zinc-950/20 space-y-2">
              ...
              <p className="text-[10px] text-zinc-400 font-medium font-mono">
                Calculated LLM invoice charges.
              </p>
            </div>
            <div className="p-6 border border-zinc-800 bg-zinc-950/20 space-y-2">
              <span className="text-[9px] font-black text-zinc-500 uppercase tracking-widest block">
            ...
            <div className="p-6 border border-zinc-800 bg-zinc-950/20 space-y-2">
              ...
              <p className="text-[10px] text-zinc-400 font-medium font-mono">
                Completed architectural queries.
              </p>
            </div>
          </div>
          ...
          {logs.length === 0 ? (
            <div className="border border-zinc-900 p-12 text-center text-nowrap flex flex-col items-center justify-center gap-2">
              ...
              <p className="text-[10px] text-zinc-600 font-medium">
                Invoke the AI Architect or sync blueprints to audit credits.
              </p>
            </div>
          ) : (
                ...
                <tbody className="divide-y divide-zinc-900 font-medium text-[11px]">
                  {logs.map((log) => (
                    ...
                    >
                      ...
                      <td className="p-4">{getProviderBadge(log.provider)}</td>
                      <td className="p-4 font-mono text-zinc-400">
                        {log.model}
                      </td>
                      <td
                        className="p-4 text-zinc-300 font-mono max-w-[200px] truncate"
                        title={log.promptSummary}
                      >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="flex items-center justify-between p-6 border border-zinc-900 bg-black/30">
              <div className="space-y-1">
                ...
                <p className="text-[10px] text-zinc-500 font-medium leading-relaxed">
                  Strict validation for all flow logic and state transitions.
                </p>
              </div>
              <button
            ...
            <div className="p-6 border border-zinc-900 bg-black/30 flex items-center justify-between opacity-50 cursor-not-allowed">
              <div className="space-y-1">
                ...
                <p className="text-[10px] text-zinc-500 font-medium leading-relaxed">
                  Deploy system logic to globally distributed nodes.
                </p>
              </div>
            ...
            {generatedKey ? (
              <div className="space-y-4">
                <p className="text-xs text-zinc-400">
                  Your new API key. Please copy it now, you will not be able to
                  see it again.
                </p>
                <div className="flex items-center gap-2">
                  <Input
                    value={generatedKey}
                    readOnly
                    className="bg-black border-zinc-700 text-emerald-400 font-mono text-xs h-10"
                  />
                  <Button
                    onClick={() => {
                    ...
                    className="bg-white text-black h-10 px-4 text-xs font-bold rounded-md hover:bg-zinc-200"
                  >
                    Copy
                  </Button>
                </div>
              ...
              <div className="flex flex-col items-center justify-center py-6">
                ...
                <Button
                  onClick={() =>
                    setGeneratedKey(
                      `stm_live_${Math.random().toString(36).substring(2, 15)}${Math.random().toString(36).substring(2, 15)}`,
                    )
                  }
                  className="bg-white text-black hover:bg-zinc-200 rounded-md h-10 px-6 text-xs font-bold"
                >
                  Generate New Key
                </Button>
              </div>
