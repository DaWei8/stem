...
export function SecurityAuditView() {
  ...
  const projectSnapshot = useMemo(() => {
    return { pages, transitions, inputs, actions, outputs, tables, columns, userTypes, policies, variables }
  }, [pages, transitions, inputs, actions, outputs, tables, columns, userTypes, policies, variables])
  ...
  const flawsStats = useMemo(() => {
    return {
      ...
      optimization: localFlaws.filter(f => f.severity === 'optimization').length
    }
  }, [localFlaws])
  ...
  const getSeverityBadgeClass = (severity: 'critical' | 'warning' | 'optimization') => {
    switch (severity) {
      case 'critical':
        ...
        <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-zinc-100 dark:border-zinc-900 pb-6">
          <div className="space-y-2">
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-3xl font-black tracking-tighter text-black dark:text-white">Security & Vulnerability Audit</h1>
            </div>
            <p className="text-xs text-zinc-400 dark:text-zinc-500 font-medium">
              Validate system logic gates, row-level database security policies, and run deep AI threat-vector scans for <span className="text-black dark:text-white font-bold">{currentProject?.name || 'System'}</span>.
            </p>
          </div>
        </header>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-zinc-50/50 dark:bg-zinc-950/20 border border-zinc-100 dark:border-zinc-900/60 p-4 rounded-xl">
              <div className="space-y-1">
                ...
                <p className="text-[10px] text-zinc-400 dark:text-zinc-500 font-medium">
                  Deterministic check results computed directly from active components.
                </p>
              </div>
            ...
            <div className="space-y-4">
              {filteredFlaws.length > 0 ? (
                      ...
                      >
                        <div className="space-y-2 flex-1 min-w-0">
                          <div className="flex flex-wrap items-center gap-2">
                            <span className={cn("text-[9px] font-black uppercase tracking-wider px-2 py-0.5 rounded-full border font-mono", getSeverityBadgeClass(flaw.severity))}>
                              {flaw.severity}
                            </span>
                            <span className={cn("text-[9px] font-black uppercase tracking-wider px-2 py-0.5 rounded-full border font-mono", getCategoryBadgeClass(flaw.category))}>
                              {flaw.category}
                            </span>
                            {flaw.setupsAffected.map((setup, idx) => (
                              <span key={idx} className="text-[9px] font-bold text-zinc-400 dark:text-zinc-500 flex items-center gap-1 font-mono">
                                • {setup}
                              </span>
                            ))}
                          </div>
                        ...
                        {isExpanded && (
                          ...
                          >
                            <div className="p-4 space-y-4 text-[11px] leading-relaxed">
                              <div className="space-y-1">
                                <h4 className="font-black text-zinc-400 dark:text-zinc-500 uppercase tracking-wider text-[9px]">Vulnerability Details</h4>
                              ...
                              <div className="p-3.5 bg-zinc-100/50 dark:bg-zinc-950/60 border border-zinc-150 dark:border-zinc-850 rounded-lg space-y-2">
                                <h4 className="font-black text-zinc-800 dark:text-zinc-200 uppercase tracking-wider text-[9px] flex items-center gap-1.5">
                                  <CheckCircle2 className="size-3 text-green-500" /> Suggested Remedy
                                </h4>
                ...
                <div className="border border-dashed border-zinc-200 dark:border-zinc-800 rounded-xl p-12 text-center flex flex-col items-center justify-center space-y-3 bg-zinc-50/10 dark:bg-zinc-950/5 select-none">
                  ...
                  <div>
                    <h3 className="text-xs font-black text-black dark:text-white uppercase tracking-wider">System Architecture Validated</h3>
                    <p className="text-[10px] text-zinc-400 dark:text-zinc-500 font-medium mt-1">
                      No deterministic security gaps detected at this level of testing.
                    </p>
                  </div>
                </div>
              )}
          ...
          <div className="space-y-6 lg:border-l lg:border-zinc-100 lg:dark:border-zinc-900 lg:pl-8">
                ...
                >
                  {audits.map((report) => (
                    <option key={report.id} value={report.id}>
                      {new Date(report.created_at).toLocaleString()} (Flaws: {report.flaws_count})
                    </option>
                  ))}
                </select>
              </div>
            ...
            {isGenerating && (
              <div className="border border-dashed border-purple-500/20 rounded-xl p-12 text-center flex flex-col items-center justify-center space-y-3 bg-purple-50/[0.02] dark:bg-purple-950/[0.02] animate-pulse select-none">
                ...
                <div>
                  <h3 className="text-xs font-black text-purple-600 dark:text-purple-400 uppercase tracking-wider">Generative Audit Active</h3>
                  <p className="text-[10px] text-zinc-400 dark:text-zinc-500 font-medium mt-1 leading-relaxed">
                    AI is inspecting access policies, transitions, variables, and Tauri bindings. This may take up to a minute...
                  </p>
                </div>
              </div>
            )}
