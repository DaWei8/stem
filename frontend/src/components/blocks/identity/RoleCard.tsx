    ...
    >
      ...
      >
        ...
        {!isViewer && (
          <div className="absolute top-4.5 right-3.5 z-10">
            <DropdownMenu onOpenChange={setMenuOpen}>
              <DropdownMenuTrigger
                render={
                  <Button
                    variant="ghost"
                    size="icon"
                    className="size-8 hover:bg-zinc-100 dark:hover:bg-zinc-900 rounded-md"
                  >
              ...
              <DropdownMenuContent
                align="end"
                className="bg-white dark:bg-black border-zinc-200 dark:border-zinc-800 rounded-md shadow-xl text-black dark:text-white"
              >
                <DropdownMenuItem
                  onClick={onEdit}
          ...
          <CardTitle className="text-sm flex items-center gap-2 font-bold tracking-tight text-zinc-900 dark:text-zinc-100 transition-colors">
            ...
            {role.is_admin && !isImpersonating && (
              <div className="flex items-center gap-1.5 px-2 py-0.5 bg-red-500/10 border border-red-500/20 rounded-full">
                ...
                <span className="text-[8px] font-black uppercase tracking-wider text-red-500 dark:text-red-400">
                  System Admin
                </span>
              </div>
            )}
            ...
            {isImpersonating && (
              <div className="flex items-center gap-1.5 px-2 py-0.5 bg-amber-500/10 border border-amber-500/30 rounded-full">
                ...
                <span className="text-[9px] font-bold tracking-wider text-amber-500 font-mono">
                  Simulating flow
                </span>
              </div>
            )}
        ...
        <CardContent className="px-5 pb-2 pt-0 flex-1 flex flex-col justify-between gap-4">
          ...
          <div
            className={cn(
              "grid gap-2 mt-auto pt-3 border-t border-zinc-100 dark:border-zinc-900/60",
              isViewer ? "grid-cols-1" : "grid-cols-2",
            )}
          >
            {!isViewer && (
              <button
                ...
                className="text-xs font-bold px-2.5 py-2 rounded-md border border-zinc-200 dark:border-zinc-800 text-zinc-500 hover:text-black dark:text-zinc-400 dark:hover:text-white hover:border-zinc-400 dark:hover:border-zinc-700 transition-all duration-200 bg-white/50 dark:bg-zinc-950/50 flex items-center justify-center gap-1 shadow-sm"
              >
            ...
            <button
              ...
              title={
                isImpersonating
                  ? "Stop simulating flow"
                  : `Simulate canvas flow and screen access from the perspective of ${role.name}`
              }
              className={cn(
                "text-xs font-bold px-2.5 py-2 rounded-md border transition-all duration-200 flex items-center gap-1 shadow-sm justify-center",
                isImpersonating
            ...
            >
              ...
              />
              {isImpersonating ? "Stop Simulation" : "Simulate Flow"}
            </button>
          </div>
        </CardContent>
      </Card>
