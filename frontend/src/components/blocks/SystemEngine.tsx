...
export function SystemEngine() {
  ...
  const [selectedConstantId, setSelectedConstantId] = useState<string | null>(
    null,
  );
  ...
  const [entryType, setEntryType] = useState<
    "variable" | "constant" | "table" | "function" | "dependency"
  >("variable");

  // Variable Form
  const [varLabel, setVarLabel] = useState("");
  const [varType, setVarType] = useState<
    "string" | "number" | "boolean" | "date" | "object" | "array" | "dictionary"
  >("string");
  const [varScope, setVarScope] = useState<
    "persistent" | "transient" | "contextual"
  >("persistent");
  ...
  const [constType, setConstType] = useState<
    "string" | "number" | "boolean" | "array" | "object" | "dictionary"
  >("string");
  ...
  const tabs = [
    {
      ...
      name: "Variables",
      icon: Layers,
      count: variables.length + constants.length,
    },
  ...
  return (
    <div className="flex h-full w-full bg-white dark:bg-black transition-colors duration-300 overflow-hidden">
      <div
        className={cn(
          "flex-1 w-full overflow-y-auto p-8 space-y-8 custom-scrollbar",
          (selectedVarId ||
            selectedConstantId ||
            selectedTableId ||
            selectedFunctionId ||
        ...
        >
          <div className="flex gap-2">
            <Button
              ...
              className={cn(
                "px-4 h-10 text-xs font-bold text-nowrap rounded-md gap-2",
                isOpen
                  ? "bg-blue-700 text-white hover:bg-blue-600 border-none"
                  : "bg-white dark:bg-black border border-zinc-200 dark:border-zinc-800 text-black dark:text-white hover:bg-zinc-50 dark:hover:bg-zinc-850",
              )}
            >
            ...
            >
              ...
              <ArrowRight className="size-0 group-hover:size-3 transition-all" />
            </Button>
          </div>
        </PillarHeader>
              ...
              {activeTab === "state" && (
                <div className="flex flex-col gap-6">
                  <div className="space-y-4">
                    <h3 className="text-[10px] font-black text-zinc-400 dark:text-zinc-500 tracking-widest px-1 uppercase">
                      Registry (Variables)
                    </h3>
                    <DataStateTable
                      variables={variables}
                        ...
                        .map((c) => (
                          <ConstantCard
                            ...
                            onDelete={
                              isViewer
                                ? undefined
                                : (id) => deleteConstant(projectId, id)
                            }
                            onClick={() => {
                              setSelectedConstantId(
                                selectedConstantId === c.id ? null : c.id,
                              );
                    ...
                    .map((f) => (
                      <FunctionCard
                        ...
                        onDelete={
                          isViewer
                            ? undefined
                            : (id) => deleteFunction(projectId, id)
                        }
                        onClick={() => {
                          setSelectedFunctionId(
                            selectedFunctionId === f.id ? null : f.id,
                          );
                          setSelectedVarId(null);
                          setSelectedConstantId(null);
                          setSelectedTableId(null);
                      ...
                      >
                        ...
                        {!isViewer && (
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent
                              align="end"
                              className="bg-black border-zinc-800 text-white rounded-md"
                            >
                              <DropdownMenuItem
                                onClick={(e) => {
                                  e.stopPropagation();
                                  deleteDependency(projectId, d.id);
                                }}
      ...
      >
        <div className="space-y-6 pb-4">
          <div className="p-3 bg-amber-500/10 border border-amber-500/20 rounded-md flex items-start gap-2.5">
            ...
            <div>
              <p className="text-[10px] font-black text-amber-600 dark:text-amber-500 uppercase tracking-wide">
                Review Required
              </p>
              <p className="text-[9px] text-zinc-500 leading-normal mt-0.5">
                Please verify the configuration below. Creating duplicate
                constants, functions, or dependencies with existing identifiers
                within the project will be blocked.
              </p>
            </div>
          </div>
                ...
                <div className="space-y-1">
                  ...
                  <textarea
                    ...
                    placeholder={
                      constType === "array"
                        ? 'e.g. ["item1", "item2"]'
                        : constType === "object" || constType === "dictionary"
                          ? 'e.g. { "key": "value" }'
                          : constType === "boolean"
                            ? "true or false"
                            : "e.g. 0.15"
                  ...
                  <p className="text-[9px] text-zinc-400 font-mono">
                    {constType === "array" ||
                    constType === "object" ||
                    constType === "dictionary"
                      ? "Must be valid JSON formatted string."
                      : ""}
                  </p>
                </div>
              </>
                ...
                <div className="space-y-1">
                  ...
                  >
                    ...
                    <option value="service">
                      Cloud Infrastructure / DB Provider
                    </option>
                  </select>
                </div>
              </>
