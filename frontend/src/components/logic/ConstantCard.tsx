...
export function ConstantCard({ constant, onDelete }: ConstantCardProps) {
  return (
    <motion.div layout initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
      <Card className="bg-black/50 border-zinc-800 rounded-none shadow-none group hover:border-zinc-500 transition-all overflow-hidden">
        <CardHeader className="px-4 py-3 flex flex-row items-center justify-between space-y-0 border-b border-zinc-800/50 bg-black/20">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger render={
              <Button variant="ghost" size="icon" className="size-6 rounded-none hover:bg-zinc-800 p-0">
            ...
            <DropdownMenuContent align="end" className="bg-black border-zinc-800 text-white rounded-none">
              <DropdownMenuItem onClick={() => onDelete(constant.id)} className="text-red-400 hover:bg-red-950 rounded-none text-xs font-bold py-2 cursor-pointer">
